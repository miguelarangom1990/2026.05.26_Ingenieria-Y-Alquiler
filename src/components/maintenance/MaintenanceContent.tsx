import React from 'react';
import { KanbanSquare, ListFilter, Wrench, Trash2 } from 'lucide-react';
import { MaintenanceCard, MaintenanceStatus, MaintenanceColumn, FilterRule, SortRule } from '../../types';
import { MaintenanceKanbanView } from './MaintenanceKanbanView';
import { MaintenanceListView } from './MaintenanceListView';
import { MaintenanceTableView } from './MaintenanceTableView';

interface MaintenanceContentProps {
  currentSection: string;
  viewMode: 'dashboard' | 'list' | 'table';
  historyMode: 'active' | 'history';
  maintenanceCards: MaintenanceCard[];
  sortedMaintenanceHistory: MaintenanceCard[];
  processedMaintenanceData: { groups: { key: string; label: string; items: MaintenanceCard[] }[] };
  selectedMaintenanceIds: Set<string>;
  setSelectedMaintenanceIds: React.Dispatch<React.SetStateAction<Set<string>>>;
  deleteMaintenanceCard: (id: string) => void;
  setConfirmDialog: (config: { isOpen: boolean; message: string; onConfirm: () => void }) => void;
  setViewingMaintenanceCard: (card: MaintenanceCard | null) => void;
  setMaintenanceViewMode: (mode: 'detailed' | 'simplified') => void;
  MAINTENANCE_COLUMNS: MaintenanceColumn[];
  generateMaintenanceTitle: (items: any[], client?: string) => string;
  setMaintenanceCards: React.Dispatch<React.SetStateAction<MaintenanceCard[]>>;
  updateMaintenanceStatus: (id: string, status: MaintenanceStatus) => void;
  updateMaintenanceItems: (id: string, items: any[]) => void;
  maintTableColOrder: string[];
  setMaintTableColOrder: React.Dispatch<React.SetStateAction<string[]>>;
  maintDraggedColId: string | null;
  setMaintDraggedColId: React.Dispatch<React.SetStateAction<string | null>>;
  columnWidths: Record<string, number>;
  setColumnWidths: React.Dispatch<React.SetStateAction<Record<string, number>>>;
  activeFilters: { 
    filterRules: FilterRule[];
    setFilterRules: React.Dispatch<React.SetStateAction<FilterRule[]>>;
    clearFilters: () => void;
  };
  setSelectedStatuses: React.Dispatch<React.SetStateAction<any[]>>;
  sortRules: SortRule[];
  setSortRules: (rules: SortRule[]) => void;
  toggleOrderExpansion: (id: string, e?: React.MouseEvent) => void;
  expandedOrders: Set<string>;
  isModalOpen: boolean;
}

export const MaintenanceContent: React.FC<MaintenanceContentProps> = React.memo(({
  currentSection, viewMode, historyMode, maintenanceCards, sortedMaintenanceHistory,
  processedMaintenanceData, selectedMaintenanceIds, setSelectedMaintenanceIds,
  deleteMaintenanceCard, setConfirmDialog, setViewingMaintenanceCard,
  setMaintenanceViewMode, MAINTENANCE_COLUMNS, generateMaintenanceTitle,
  setMaintenanceCards, updateMaintenanceStatus, updateMaintenanceItems,
  maintTableColOrder, setMaintTableColOrder, maintDraggedColId, setMaintDraggedColId,
  columnWidths, setColumnWidths, activeFilters, setSelectedStatuses,
  sortRules, setSortRules, toggleOrderExpansion, expandedOrders, isModalOpen
}) => {
  if (currentSection !== 'maintenance') return null;

  const maintenanceCount = maintenanceCards.filter(c => 
    historyMode === 'history' 
      ? ['COMPLETADO', 'CANCELADO'].includes(c.status) 
      : !['COMPLETADO', 'CANCELADO'].includes(c.status)
  ).length;

  const renderEmptyState = (icon: React.ReactNode, message: string, showClearFilters = false) => (
    <div className={`mt-4 mb-4 ${isModalOpen ? 'hidden' : ''}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
           {viewMode === 'dashboard' ? <KanbanSquare className="w-6 h-6 text-slate-400" /> : <ListFilter className="w-6 h-6 text-slate-400" />}
           <h2 className="text-2xl font-bold text-slate-900">
             {historyMode === 'history'
               ? (viewMode === 'dashboard' ? 'Kanban de Historial Mantenimiento' : viewMode === 'table' ? 'Tabla de Historial Mantenimiento' : 'Lista de Historial Mantenimiento')
               : (viewMode === 'dashboard' ? 'Tablero Kanban Mantenimiento' : viewMode === 'table' ? 'Tabla de Mantenimientos' : 'Lista de Mantenimientos')}
           </h2>
        </div>
      </div>
      <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-slate-300">
        {icon}
        <p className="text-slate-500 mb-4">{message}</p>
        {showClearFilters && (
          <button 
            onClick={() => {
              activeFilters.clearFilters();
              setSelectedStatuses([]);
            }}
            className="text-indigo-600 font-bold hover:underline"
          >
            Quitar filtros
          </button>
        )}
      </div>
    </div>
  );

  if (maintenanceCount === 0) {
    return renderEmptyState(<Wrench className="w-12 h-12 text-slate-200 mx-auto mb-4" />, "No hay mantenimientos registrados aún.");
  }

  if (sortedMaintenanceHistory.length === 0) {
    return renderEmptyState(<ListFilter className="w-12 h-12 text-slate-200 mx-auto mb-4" />, "No se encontraron resultados con los filtros aplicados.", true);
  }

  return (
    <div className={`flex flex-col flex-1 min-h-0 ${isModalOpen ? 'hidden' : ''}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          {viewMode === 'dashboard' ? <KanbanSquare className="w-6 h-6 text-slate-400" /> : <ListFilter className="w-6 h-6 text-slate-400" />}
          <h2 className="text-2xl font-bold text-slate-900">
            {historyMode === 'history'
              ? (viewMode === 'dashboard' ? 'Kanban de Historial Mantenimiento' : viewMode === 'table' ? 'Tabla de Historial Mantenimiento' : 'Lista de Historial Mantenimiento')
              : (viewMode === 'dashboard' ? 'Tablero Kanban Mantenimiento' : viewMode === 'table' ? 'Tabla de Mantenimientos' : 'Lista de Mantenimientos')}
          </h2>
        </div>
        <div className="flex items-center gap-2">
          {viewMode === 'table' && selectedMaintenanceIds.size > 0 && (
            <button
              onClick={() => {
                setConfirmDialog({
                  isOpen: true,
                  message: `¿Estás seguro de que deseas eliminar ${selectedMaintenanceIds.size} mantenimiento(s) seleccionado(s)? Esta acción no se puede deshacer.`,
                  onConfirm: () => {
                    selectedMaintenanceIds.forEach(id => deleteMaintenanceCard(id));
                    setSelectedMaintenanceIds(new Set());
                  }
                });
              }}
              className="flex items-center gap-2 text-xs font-bold text-red-600 hover:text-red-700 uppercase tracking-widest bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-all border border-red-100 shadow-sm"
            >
              <Trash2 className="w-3.5 h-3.5" />
              Eliminar seleccionados ({selectedMaintenanceIds.size})
            </button>
          )}
        </div>
      </div>

      {viewMode === 'dashboard' ? (
        <MaintenanceKanbanView
          historyMode={historyMode}
          sortedMaintenanceHistory={sortedMaintenanceHistory}
          setViewingMaintenanceCard={setViewingMaintenanceCard}
          setMaintenanceViewMode={setMaintenanceViewMode}
          setConfirmDialog={setConfirmDialog}
          deleteMaintenanceCard={deleteMaintenanceCard}
          MAINTENANCE_COLUMNS={MAINTENANCE_COLUMNS}
          generateMaintenanceTitle={generateMaintenanceTitle}
          setMaintenanceCards={setMaintenanceCards}
        />
      ) : viewMode === 'list' ? (
        <MaintenanceListView
          processedMaintenanceData={processedMaintenanceData}
          setViewingMaintenanceCard={setViewingMaintenanceCard}
          setMaintenanceViewMode={setMaintenanceViewMode}
          MAINTENANCE_COLUMNS={MAINTENANCE_COLUMNS}
          generateMaintenanceTitle={generateMaintenanceTitle}
          updateMaintenanceStatus={updateMaintenanceStatus}
          setConfirmDialog={setConfirmDialog}
          deleteMaintenanceCard={deleteMaintenanceCard}
        />
      ) : (
        <MaintenanceTableView
          processedMaintenanceData={processedMaintenanceData}
          selectedMaintenanceIds={selectedMaintenanceIds}
          setSelectedMaintenanceIds={setSelectedMaintenanceIds}
          setViewingMaintenanceCard={setViewingMaintenanceCard}
          setMaintenanceViewMode={setMaintenanceViewMode}
          maintTableColOrder={maintTableColOrder}
          setMaintTableColOrder={setMaintTableColOrder}
          maintDraggedColId={maintDraggedColId}
          setMaintDraggedColId={setMaintDraggedColId}
          columnWidths={columnWidths}
          setColumnWidths={setColumnWidths}
          toggleOrderExpansion={toggleOrderExpansion}
          expandedOrders={expandedOrders}
          sortRules={sortRules}
          setSortRules={setSortRules}
        />
      )}
    </div>
  );
});
